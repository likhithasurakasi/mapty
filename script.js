...
class App {
  ...
  static validInputs = (...inputs) => inputs.every(inp => Number.isFinite(inp));
...
<img
  ...
  _getLocalStorage() {
    ...
    this.#workouts = this.#workouts.map(w => {
      ...
      // prettier-ignore
      if (w.type == 'running')
        w = new Running(w.coords,w.distance,w.duration,w.cadence,w.time,w.tunit,w.temp,w.icon,w.city,w.country
        );
